...
var splashBoxEl = $('.splash__box-bg');
var splashBoxAnim = TweenMax.to(splashBoxEl, 1, {
  y: '15%',
  rotation: '-4deg'
});
var splashBoxScene = new ScrollMagic.Scene({
  triggerElement: splashBoxEl,
  duration: '125%'
}).setTween(splashBoxAnim).addTo(controller);

if (window.innerWidth >= 768) {

  $('.intro__column').each(function (index, element) {
    var tween = TweenMax.to(element, 1, { y: -200 / (index + 2) + '%' });
